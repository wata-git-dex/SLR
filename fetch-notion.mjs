...
for (const p of strainPages) {
  ...
  let   has_image = existsSync(jpgPath);
  ...
  if (!has_image) {
    ...
    if (imgUrl) {
      try {
        ...
        console.log("  SKIP " + name + ": " + e.message.slice(0, 100));
      }
    } else {
      console.log("  NO PHOTO " + name);
    }
  } else {
...
// Update the landing page date (the hardcoded "Aug 7" text)
const now = new Date();
